        ...
        <div class="device-display" @mouseover="showDeviceInfo = true" @mouseleave="showDeviceInfo = false">
            <div v-if="currentContent.deviceData" class="image-container">
                ...
                <template v-if="currentContent.deviceType === 'optical'">
                    <img src="@/assets/dianzhen.gif" alt="光电数据" :class="['device-image', { 'large-image': isLarge }]" />
                </template>
...
// 设备流映射
const deviceStreams = ref(new Map());
...
// WebRTC 相关方法
const initWebRTC = async (deviceId) => {
    try {
        ...
        // 获取设备对应的流地址
        const streamUrl = deviceStreams.value.get(deviceId);
        if (!streamUrl) return;
...
const loadDevices = async () => {
    // 模拟从API加载设备列表
    devices.value = [
        { id: 1, name: '设备1', streamUrl: 'rtsp://camera1/stream' },
        { id: 2, name: '设备2', streamUrl: 'rtsp://camera2/stream' },
        { id: 3, name: '设备3', streamUrl: 'rtsp://camera3/stream' }
    ];

    // 更新设备流映射
    deviceStreams.value.clear();
    devices.value.forEach(device => {
        deviceStreams.value.set(device.id, device.streamUrl);
    });
...
const loadDeviceData = async () => {
    // 模拟从API加载设备数据
    deviceData.value = {
        name: `${selectedDeviceType.value}设备${selectedDeviceId.value}`,
        status: '正常',
        model: 'XYZ-123',
    ...
    if (selectedDeviceType.value === 'video') {
        await initWebRTC(selectedDeviceId.value);
    }
